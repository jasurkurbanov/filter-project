...
allUsers.onclick = function () {
  renderUser(CARS_DATA);
};

oneToFive.onclick = function () {
  const filteredArray = CARS_DATA.filter((item) => item.id > 9600);
  renderUser(filteredArray);
};

btnFiveToTen.onclick = function () {
  const filteredArray = CARS_DATA.filter((item) => item.make === "GMC");
  renderUser(filteredArray);
};

tenToFifteen.onclick = function () {
  const filteredArray = CARS_DATA.filter((item) => item.year < 2010);

  renderUser(filteredArray);
};

fifteenToFifty.onclick = function () {
  const filteredArray = CARS_DATA.filter((item) => item.type.includes("Sedan"));
  renderUser(filteredArray);
};
